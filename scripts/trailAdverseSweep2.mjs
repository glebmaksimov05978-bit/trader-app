...
function simulate(candles, series, entryIndex, dir, entryPrice, atr, mode) {
  ...
  for (let i = entryIndex + 1; i <= end; i++) {
    ...
    if ((mode === 'score' || mode === 'score+traj') && !evaluated && -ret >= minPeakPct) {
      evaluated = true;
...
const MODES = [
  ['Базовый трейл', 'trail'],
  ['Счёт>=3 (вчерашняя лучшая находка)', 'score'],
  ['Счёт>=3 + траектория (7-й компонент)', 'score+traj'],
  ['Только траектория (медиана, 3 бара подряд)', 'traj-median-3bar'],
];

console.log(`\n${'='.repeat(120)}`);
console.log('ДЕНЬГИ: каждый режим — С фильтром рынка и БЕЗ (портфель риск10%/8 слотов, отложенный период)');
console.log(`${'='.repeat(120)}\n`);
console.log('Вариант                                        | БЕЗ фильтра рынка        | С ФИЛЬТРОМ РЫНКА');
console.log('-'.repeat(120));
for (const [label, mode] of MODES) {
  ...
  const withoutFilter = results;
  const withFilter = results.filter((r) => !(r.dir === 1 && r.marketDown));

  const pNo = runPortfolioBacktest(toPortfolio(withoutFilter), { startingCapital: 100000, riskPerTradePct: 10, maxConcurrentPositions: 8 });
  ...
  console.log(
    `${label.padEnd(48)} | ${((pNo.totalReturnPct >= 0 ? '+' : '') + pNo.totalReturnPct.toFixed(1)).padStart(6)}% просад.${pNo.maxDrawdownPct.toFixed(1).padStart(5)}% n=${String(withoutFilter.length).padStart(3)} | `
    + `${((pYes.totalReturnPct >= 0 ? '+' : '') + pYes.totalReturnPct.toFixed(1)).padStart(6)}% просад.${pYes.maxDrawdownPct.toFixed(1).padStart(5)}% n=${String(withFilter.length).padStart(3)}`
  );
}
