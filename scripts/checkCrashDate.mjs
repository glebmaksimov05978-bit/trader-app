...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'traderpro-check-'));
...
const candles = await fetchDailyCandles({ ticker: 'IMOEXF', instrumentType: 'future', toDate: new Date(), timeframe: 'D1', lookbackDays: 2200 });
const june = candles.filter((c) => new Date(c.date) >= new Date('2026-06-15') && new Date(c.date) <= new Date('2026-07-15'));
for (const c of june) console.log(new Date(c.date).toISOString().slice(0, 10), 'close:', c.close.toFixed(1));
