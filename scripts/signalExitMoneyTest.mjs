// Проверка НОВОГО правила выхода `signalExitEnabled` (2026-08-14) — оно ДОКАЗАНО
// предсказывает откат/разворот (holdout на D1 и H1), но это не то же самое, что
// "зарабатывает деньги". Прогоняем через реальный движок + портфельный режим (честный
// расчёт денег, см. src/services/backtest/portfolio.js) и сравниваем с классикой и
// сегодняшним симметричным трейлом.
import fs from 'fs';
import os from 'os';
...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'traderpro-signalmoney-'));
...
const CONFIGS = [
  { name: 'A. Классика 2%/4%', rules: { ...base, stopType: 'pct', stopPct: 2, takeType: 'pct', takePct: 4, trailEnabled: false, signalExitEnabled: false } },
  { name: 'Е. Симметричный трейл (вчера)', rules: { ...base, stopType: 'none', takeType: 'none', trailEnabled: true, trailAdverseEnabled: true, trailAdverseMult: 3, signalExitEnabled: false } },
  { name: 'F. Трейл + сигнальный выход (сегодня)', rules: { ...base, stopType: 'none', takeType: 'none', trailEnabled: true, trailAdverseEnabled: true, trailAdverseMult: 3, signalExitEnabled: true } },
  { name: 'G. Только сигнальный выход (без блант-трейла)', rules: { ...base, stopType: 'none', takeType: 'none', trailEnabled: true, trailAdverseEnabled: false, signalExitEnabled: true } },
];
...
console.log(`\n${'='.repeat(115)}`);
console.log('Новый сигнальный выход (RSI+EMA динамика) — под портфельным режимом (риск 10% / до 8 позиций)');
console.log(`${'='.repeat(115)}\n`);
console.log('Вариант                                        | Период | Доходность | Просадка | Закрыто | Открыто | Худшая | Причины');
...
for (const cfg of CONFIGS) {
  ...
  for (const { ticker, candles } of candlesByTicker) {
    ...
    for (const t of test.trades) if (t.status === 'closed') reasonCounts[t.exitReason] = (reasonCounts[t.exitReason] || 0) + 1;
  }
  for (const [label, byTicker] of [['обучен', trainByTicker], ['ОТЛОЖ.', testByTicker]]) {
    ...
    console.log(
      `${(label === 'обучен' ? cfg.name : '').padEnd(46)} | ${label} | `
      ...
      + `${r.worstTrade.toFixed(1)}%`.padStart(6) + ` | ${label === 'ОТЛОЖ.' ? JSON.stringify(reasonCounts) : ''}`
    );
  }
  console.log('-'.repeat(120));
}
