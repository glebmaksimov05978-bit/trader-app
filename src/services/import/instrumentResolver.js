...
// Тип инструмента по самому коду тикера — один резолвер на всё приложение.
//
// Раньше эта тройка условий жила локальной функцией внутри Journal.js, и Калькулятор про
// неё не знал: он брал тип из переключателя «⚡ Фьючерс / 📈 Акция», который по умолчанию
// стоит на фьючерсе. Из-за этого у акции (реальный случай — SVCB, биржа прямо подписала
// «Акция MOEX») комиссия считалась по ФЬЮЧЕРСНОЙ ставке тарифа: 0.04% вместо 0.05% на
// «Трейдере». Хуже того, неверный тип уезжал в саму сделку и жил там дальше — по нему
// потом считались и закрытие, и Сопровождение.
export function guessInstrumentType(code) {
  if (isCurrencyCode(code)) return 'currency';
  if (isFuturesCode(code)) return 'future';
  return 'stock';
}
