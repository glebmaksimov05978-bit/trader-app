...
// Подтверждено живым ответом Т-Банка (2026-09-13): logoName приходит УЖЕ с расширением —
// например "RU0007661625.png" (ISIN Газпрома) или "tcs2.png" — никакой суффикс размера
// вроде "x160" приписывать не нужно, имя файла берётся как есть. Первая версия этого кода
// сама дописывала "x160.png" поверх уже готового имени и получала несуществующий файл
// вроде "RU0007661625.pngx160.png" — отсюда были одни заглушки вместо логотипов.
const LOGO_CDN = 'https://invest-brands.cdn-tinkoff.ru';

export function logoUrlFromName(logoName) {
  if (!logoName) return null;
  // Защита на случай, если когда-нибудь придёт имя без расширения — не наблюдалось живьём,
  // но лучше не собирать заведомо битый адрес.
  const file = /\.(png|svg|jpe?g)$/i.test(logoName) ? logoName : `${logoName}.png`;
