...
// Торговая сессия Мосбиржи в UTC (10:00-18:40 МСК). Вне неё проверять нечего, а на
// серверной схеме это ещё и прямая экономия запросов.
export function isTradingHours(date = new Date()) {
  const day = date.getUTCDay();
  if (day === 0 || day === 6) return false;
  const minutes = date.getUTCHours() * 60 + date.getUTCMinutes();
  return minutes >= 7 * 60 && minutes <= 15 * 60 + 40;
}
  ...
  if (prefs[ALERT_TYPES.LOSS_SIGNAL] && now.lossScore != null && now.lossScore <= lossThreshold) {
    out.push({
      ...
      body: `Убыток ${pct(now.returnPct)}. ${moveLine}\n\n`
        + `Признаков того, что дно рядом, почти нет (${now.lossScore} из ${lossThreshold}): `
