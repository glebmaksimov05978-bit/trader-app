...
export default function Calculator() {
  ...
  // Тип инструмента: фьючерс или акция
  ...
  // Сколько контрактов реально используем
  const effectiveContracts = manualContracts
    ? Math.max(1, parseInt(manualContracts) || 1)
    : (result?.contracts || 1);

  // Пересчёт детализации с учётом ручного ввода
      ...
      <div className="calc-layout">
        ...
        <div className="calc-input-panel">
          ...
          <div style={{marginBottom:20}}>
            ...
            {instrumentInfo && (
              <div className="instrument-info" style={{marginTop:10}}>
                <span className="badge badge-purple">{instrumentInfo.ticker}</span>
                <span className="text-sm text-secondary" style={{marginLeft:8}}>{instrumentInfo.name}</span>
                {instrumentInfo.isShare ? (
                  <span className="text-xs text-muted" style={{marginLeft:8}}>📈 Акция MOEX</span>
                ) : (
                ...
                <span className="text-xs text-muted" style={{marginLeft:8, color:'var(--green)'}}>🔄 авто-обновление 30с</span>
              </div>
            )}
          </div>
          ...
          <div style={{marginBottom:20}}>
            ...
            <div style={{display:'flex', gap:8}}>
              {['Лонг','Шорт'].map(d => (
                <button key={d}
                  className={form.entryPrice && result?.direction === d.toLowerCase() ? 'btn btn-primary' : 'btn btn-secondary'}
                  style={{flex:1}}
                  onClick={() => {
                    // меняем sl чтобы зафиксировать направление
                    if (d === 'Лонг' && form.stopLoss > form.entryPrice) set('stopLoss', '');
                    if (d === 'Шорт' && form.stopLoss < form.entryPrice) set('stopLoss', '');
                  }}
                >
                  {d === 'Лонг' ? '✅' : '🔽'} {d}
                </button>
            ...
            <div className="calc-grid-2">
              ...
              <div className="input-group">
                ...
                <div className="input-prefix">
                  ...
                  <input className="input" type="number" value={form.riskPercent}
          ...
          <div>
            ...
            {instrumentType === 'future' && (
              <div className="calc-grid-2" style={{marginTop:12}}>
                <div className="input-group">
            ...
            <div className="input-group" style={{marginTop:12}}>
              ...
              <input className="input" type="number" value={form.commissionRate}
        ...
        <div className="calc-results-panel">
          {result && displayResult && result.contracts > 0 ? (
            <>
              ...
              <div className="calc-key-metrics" style={{marginBottom:16}}>
                ...
                <div className={`calc-metric-card ${displayResult?.direction === 'long' ? 'green' : 'red'}`}>
                  ...
                  <div style={{
                    ...
                    <span style={{fontSize:20, fontWeight:800, color: manualContracts ? 'var(--text-muted)' : 'var(--text-primary)'}}>
                      {result?.contracts}
                    </span>
                ...
                <div className={`calc-metric-card ${!displayResult?.rrValid && displayResult?.rr !== 0 ? 'red' : displayResult?.rr >= 2 ? 'green' : displayResult?.rr >= 1 ? 'gold' : 'red'}`}>
                  ...
                  <div style={{fontSize:32, fontWeight:800, color: rrColor}}>
              ...
              {/* Прогресс-бар использования капитала */}
              <div className="card" style={{marginTop:16}}>
                ...
                <div style={{
                  height:8, borderRadius:4, background:'var(--bg-surface-2)',
                ...
                {(displayResult?.marginUsagePercent || 0) > 70 ? (
                  <div className="text-xs text-muted" style={{marginTop:6, color:'var(--gold)'}}>⚠️ Высокая загрузка депозита — рискованно</div>
                ) : (displayResult?.marginUsagePercent || 0) > 40 ? (
                  <div className="text-xs text-muted" style={{marginTop:6, color:'var(--gold)'}}>🟡 Умеренная загрузка депозита</div>
                ) : (
                  <div className="text-xs text-muted" style={{marginTop:6, color:'var(--green)'}}>✅ Нормальная загрузка</div>
                )}
              </div>
            </>
          ) : (
