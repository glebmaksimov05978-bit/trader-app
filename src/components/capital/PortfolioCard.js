...
export default function PortfolioCard({ userProfile, strategies, trades, onSave }) {
  ...
  return (
    <div className="card" style={{ marginTop: 24 }}>
      ...
      {/* Отдельный пример на конкретных цифрах — объяснение словами выше уже задавали
          несколько раз, и вопрос «как пользоваться сразу несколькими» повторялся. Это
          НЕ режим, где две стратегии ведут ОДНУ сделку вместе, — вот что путало. */}
      <div style={{
        marginBottom: 16, padding: '12px 14px', borderRadius: 'var(--radius-sm)',
        background: 'var(--bg-surface-2)', fontSize: 12.5, lineHeight: 1.7, color: 'var(--text-secondary)',
      }}>
        <b style={{ color: 'var(--text-primary)' }}>Как это выглядит на практике.</b> Портфель не
        сводит две стратегии в одну сделку — каждая сделка по-прежнему открывается ровно по
        ОДНОЙ стратегии (выбираете в Калькуляторе). Портфель только делит депозит на именованные
        доли — и от доли считается риск для сделок этой стратегии.
        <br /><br />
        Пример на 1 000 000 ₽ с долями 60/40 (Пробойная/Откатная): открыли сделку в Калькуляторе
        по «Пробойной» — риск 1% посчитается от 600 000 ₽ (6 000 ₽), а не от всего миллиона. В
        это же время может быть открыта ДРУГАЯ сделка по «Откатной» — у неё риск считается от
        своих 400 000 ₽. Обе сделки видны в Журнале и Сопровождении как обычно, каждая ведётся
        по своей стратегии (свой профит-/лосс-счёт, свои правила выхода) — портфель просто не
        даёт «Откатной» рисковать деньгами «Пробойной», даже если та в этот момент в просадке.
      </div>
